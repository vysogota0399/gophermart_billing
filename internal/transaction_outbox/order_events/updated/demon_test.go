...
import (
	...
	"github.com/vysogota0399/gophermart_billing/internal/transaction_outbox/order_events/updated/mocks"
)

func TestDaemon_processEvent(t *testing.T) {
	type fields struct {
		events *mocks.MockOrderUpdatedEventsRepository
		pub    *mocks.MockOrderUpdatedEventsPublisher
	}
	tests := []struct {
		name      string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fields := tt.fields
			rep := mocks.NewMockOrderUpdatedEventsRepository(ctrl)
			pub := mocks.NewMockOrderUpdatedEventsPublisher(ctrl)
