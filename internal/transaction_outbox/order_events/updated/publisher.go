...
func (p *Publisher) Publish(ctx context.Context, e *models.OrderEvent) error {
	...
	if err != nil {
		return fmt.Errorf("order_events/updated/publisher: marashal failed  %w", err)
	}
	...
	if err := p.writer.WriteMessages(
		...
		return fmt.Errorf("order_events/updated/publisher: write message to toppic failed %w", err)
	}
